...
// www/src/thermoloop-panel.js
import { LitElement, html, css } from "lit";
import { classMap } from "lit/directives/class-map.js";
var RANGE_LABELS = { "6h": "6h", "24h": "24h", "7d": "7d" };
var RANGE_MS = { "6h": 6 * 3600 * 1e3, "24h": 24 * 3600 * 1e3, "7d": 7 * 24 * 3600 * 1e3 };
var ThermoLoopPanel = class extends LitElement {
  constructor() {
    super();
    this._range = "24h";
    ...
    try {
      ...
      for (const [entityId, changes] of Object.entries(result)) {
        ...
        history[bucket] = changes.map((c) => ({
          t: new Date(c.last_changed).getTime(),
          v: parseFloat(c.state)
        })).filter((p) => !isNaN(p.v));
      }
  ...
  async _fetchEvents() {
    ...
    try {
      const result = await this.hass.callWS({
        ...
        start_time: start.toISOString(),
        end_time: now.toISOString(),
        event_types: ["thermoloop_command"],
        entity_ids: []
      });
      const events = [];
      for (const event of result) {
        if (event.event_type === "thermoloop_command") {
          events.push({
            time: new Date(event.time).toLocaleTimeString(),
            detail: event.message || event.title || JSON.stringify(event),
            type: "command"
          });
        }
      }
      ...
      console.warn("ThermoLoop: logbook fetch failed", e);
    }
  }
  _findEntity(prefix) {
  ...
  _renderGraph() {
    ...
    if (target && target !== "\u2014") {
      ...
      ctx.textAlign = "left";
      ctx.fillText(`Target ${target}\xB0C`, w - pad.right - 70, targetY - 4);
    }
  }
  _rangeHistory(range) {
    this._range = range;
      ...
      <div class="grid">
        ...
        <div class="status">
          ...
          <div class="status-item">
            ...
            <span class="status-value">${statusTemp != null ? `${statusTemp}\xB0C` : "\u2014"}</span>
          </div>
          <div class="status-item">
            <span class="status-label">Target</span>
            <span class="status-value">${statusTarget != null ? `${statusTarget}\xB0C` : "\u2014"}</span>
          </div>
          <div class="status-item">
        ...
        <div class="log-card ${classMap({ collapsed: this._logCollapsed })}">
          ...
          <div class="log-entries">
            ${this._events.length === 0 ? html`<div class="log-entry"><span style="opacity:0.4">No events in this period</span></div>` : this._events.map((e) => html`
...
__publicField(ThermoLoopPanel, "styles", css`
    :host {
      display: block;
      padding: 16px;
  ...
  `);
__publicField(ThermoLoopPanel, "properties", {
  hass: { type: Object },
  config: { type: Object },
  _range: { state: true },
